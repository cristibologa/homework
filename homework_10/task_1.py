...
def task_3():
    return [i for i in range(1, 11) if not i % 2 == 0]
# CODUL TĂU VINE MAI SUS:
...
def task_4(arr):
    return [num for row in arr for num in row]
# CODUL TĂU VINE MAI SUS:
...
def task_5(n):
    return ["par" if num % 2 == 0 else "impar" for num in range(1, n+1)]
...
def task_7(n):
    return {i*3 for i in range(1, n) if i*3 < n}
# CODUL TĂU VINE MAI SUS:
...
def task_10(nume, varsta, oras="Oras"):
